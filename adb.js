    ...
    setInterval(() => {
        ...
        .then(res => {
            for(let r in res.devices) {
                ...
                .then(status => {
                    // Log device stastus to db
                    res.devices[r]['state'] = status
                    // socket emit event???                    
                    // socket emit (ip, status)
                    // Client will io.on(ip, 'deviceStatus', status)
                    console.log(res.devices[r])
                    poll.update(res.devices[r])
                    this.getScreen(currentDevice).then(
                        console.log('Succesfully screen captured '+currentDevice)
                    )
                })
            }
